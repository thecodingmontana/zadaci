...
const { data: workspaces, status: workspacesStatus, error: workspacesError, refetch: refreshWorkspaces } = useWorkspaces();
...
<template>
    <aside class="flex h-full w-full flex-col items-center space-y-3 border-r py-3 text-primary">
        ...
        <ScrollArea class="w-full flex-1">
            <div v-if="workspacesStatus === 'pending' || workspacesStatus === 'error'"
            ...
            <template v-else>
                <div v-for="workspace in workspaces" :key="workspace.id" class="mb-4">
                    <NavigationItem :id="workspace.id" :name="workspace.name" :image-url="workspace.imageUrl" />
                </div>
            </template>
        </ScrollArea>
