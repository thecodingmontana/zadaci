<script setup lang="ts" generic="TData extends TeammatesWithProfile, TValue">
...
import { auth2fas, roles, type ChangeTeammateRole, type TeammatesWithProfile } from '~/types'
...
const action = ref<'export' | 'refresh' | 'change-role' | 'remove' | null>(null)
...
const startTransition = (callback: () => void) => {
  ...
  try {
    ...
    // Add a small delay to mimic React's transition behavior
    setTimeout(() => {
...
const onDelete = async () => {
  ...
  if (rows.length > 0) {
    userIds = rows.map((item) => {
      return item.original.userId
    })
  }
        ...
        <div class="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
          <div class="flex items-center gap-2">
            <DataTableFacetedFilter
              ...
              :column="table.getColumn('role') as unknown as Column<TeammatesWithProfile, any>"
              title="Role"
              :options="roles"
            />
            <DataTableFacetedFilter
              v-if="table.getColumn('registered2FA')"
              :column="table.getColumn('registered2FA') as unknown as Column<TeammatesWithProfile, any>"
              title="2FA Auth"
              :options="auth2fas"
            />
          </div>
          <div class="flex flex-col gap-y-2 xl:flex-row xl:items-center xl:gap-3 xl:self-start">
            <div class="flex items-center gap-3">
              ...
              >
                <Loader
                  v-if="isLoading && action === 'change-role'"
                ...
                <Icon
                  ...
                  class="size-4"
                />
                Change {{ table.getFilteredSelectedRowModel().rows.length > 1 ? 'roles' : 'role' }} ({{ table.getFilteredSelectedRowModel().rows.length }})
              </Button>
              <Button
                v-if="table.getFilteredSelectedRowModel().rows.length > 0"
                aria-label="Delete Rows"
              ...
              >
                ...
                <Icon
                  ...
                  class="size-4"
                />
                Remove {{ table.getFilteredSelectedRowModel().rows.length > 1 ? 'users' : 'user' }}  ({{ table.getFilteredSelectedRowModel().rows.length }})
              </Button>
            </div>
            <div class="flex items-center gap-3">
              <Button
