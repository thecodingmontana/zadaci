...
const isInvalid = computed(() => Boolean(password.value) && strengthScore.value < 4);
  ...
  <div>
    <div class="space-y-2">
      ...
      <div class="relative">
        ...
        />
        <button
          class="absolute inset-y-px inset-e-px flex h-full w-9 items-center justify-center rounded-e-lg text-muted-foreground/80 transition-shadow hover:text-foreground focus-visible:border focus-visible:border-ring focus-visible:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50"
          type="button"
          :aria-label="isVisible ? 'Hide password' : 'Show password'"
          :aria-pressed="isVisible"
          aria-controls="input-51"
    ...
    >
      <div
        ...
        :class="[getStrengthColor(strengthScore)]"
    ...
    <ul class="space-y-1.5" aria-label="Password requirements">
      <li v-for="(req, index) in strength" :key="index" class="flex items-center gap-2">
        ...
        <span class="text-xs" :class="[req.met ? 'text-emerald-600' : 'text-muted-foreground']">
          {{ req.text }}
          <span class="sr-only">
