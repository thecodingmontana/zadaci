...
import { AlertCircle, ArrowRight, Loader, Mail, RotateCw } from "@lucide/vue";
...
<template>
  <form class="mt-5 space-y-4" @submit.prevent="onSendUniqueCode">
        ...
        <FormControl>
          ...
          >
            <input
              ...
              class="block h-11.5 w-full rounded-md border-0 bg-transparent px-3 py-2 text-sm placeholder:text-muted-foreground focus:bg-none focus:outline-none active:bg-transparent"
            />
          </div>
        </FormControl>
    ...
    <button
      type="submit"
      :disabled="
        !!sendUniqueCodeForm.controlledValues.value.email === false ||
