...
const onSubmit = form.handleSubmit((values) => {
  ...
  toast.promise(promise, {
    ...
    error: (err: any) =>
      err?.response?._data?.message ?? err?.message ?? "Sign up failed, please try again.",
...
<template>
  <form class="space-y-2" @submit.prevent="onSubmit">
    <div class="space-y-1">
      <div class="space-y-1">
        <label class="flex items-center gap-1.5 text-sm font-medium"> Email </label>
      ...
      <FormField v-slot="{ componentField }" name="code">
        <FormItem>
          <FormLabel class="flex items-center gap-1.5 text-sm font-medium"> Code </FormLabel>
          <FormControl>
            <div
    ...
    <button
      type="submit"
      :disabled="
        !form.controlledValues.value.code || !!form.errors.value.code || isSigningUp || isResendCode
      "
      :class="
        cn(
      ...
      "
    >
      <Loader2 v-if="isSigningUp" class="size-5 animate-spin" />
