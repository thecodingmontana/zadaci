...
export function useProjectSync(workspaceId: () => string | undefined) {
  let replicationState: RxReplicationState<ProjectDocType, { updated_at: string; id: string }> | null =
    null;
    ...
    replicationState = replicateRxCollection<ProjectDocType, { updated_at: string; id: string }>(
      {
        ...
        pull: {
          handler: async (checkpoint, batchSize) => {
            ...
            const result = await $fetch(`/api/replication/projects/pull?${params.toString()}`);
            return result as {
        ...
        push: {
          handler: async (rows) => {
            ...
            const result = await $fetch(`/api/replication/projects/push?workspace_id=${id}`, {
              method: "POST",
              body: rows,
            });
