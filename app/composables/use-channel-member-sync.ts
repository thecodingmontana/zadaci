...
import type { ChannelMemberDocType, ZadaciDatabase } from "~/plugins/rxdb.client";
...
export function useChannelMemberSync(workspaceId: () => string | undefined) {
  ...
  async function start() {
    ...
    const subActive = replicationState.active$.subscribe((a) =>
      console.log(`[useChannelMemberSync] Active:`, a),
    );
  ...
  function setupRealtimeChannel() {
    ...
    realtimeChannel = supabase
      .channel("app_channel_members_realtime")
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "app_channel_members",
        },
        () => {
          scheduleReSync();
        },
  ...
  function stop() {
    ...
    const timer = debounceTimers.get("channel_members");
    if (timer) {
      clearTimeout(timer);
      debounceTimers.delete("channel_members");
    }
