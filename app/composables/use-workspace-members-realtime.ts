...
import { workspaceMembersKey } from "./use-workspace-members";
...
export function useWorkspaceMembersRealtime(workspaceId: () => string | undefined) {
    ...
    channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          event: "*",
          schema: "public",
          table: "app_workspace_members",
          filter: `workspace_id=eq.${id}`,
        },
        () => {
          queryClient.invalidateQueries({ queryKey: workspaceMembersKey(id) });
        },
      )
      .subscribe((status) => {
