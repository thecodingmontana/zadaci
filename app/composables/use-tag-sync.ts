...
import type { TagDocType, ZadaciDatabase } from "~/plugins/rxdb.client";
...
export function useTagSync(workspaceId: () => string | undefined) {
  let replicationState: RxReplicationState<TagDocType, { updated_at: string; id: string }> | null =
    null;
    ...
    replicationState = replicateRxCollection<TagDocType, { updated_at: string; id: string }>({
      ...
      pull: {
        handler: async (checkpoint, batchSize) => {
          ...
          if (checkpoint) {
            const localCount = await db.tags.count().exec();
  ...
  function setupRealtimeChannel() {
    ...
    realtimeChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log("[rxdb-debug] Realtime channel fired for app_tag", payload);
          scheduleReSync();
        },
      )
      .subscribe((status) => {
        console.log("[rxdb-debug] Realtime channel subscribe status (tags):", status);
  ...
  function stop() {
    ...
    const timer = debounceTimers.get("tags");
    if (timer) {
      clearTimeout(timer);
      debounceTimers.delete("tags");
    }
