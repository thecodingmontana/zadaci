...
import InviteMemberForm from "~/components/workspace/forms/invite-member-form.vue";
...
const isModalOpen = computed(() => {
  return modalStore?.type === "inviteMember" && modalStore?.isOpen;
});
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="grid">
            ...
            <AlertDialogDescription>
              ...
              <strong class="capitalize"> {{ workspace?.name }} Workspace </strong>
              .
            </AlertDialogDescription>
          </div>
        </div>
      </AlertDialogHeader>
      <InviteMemberForm :on-close="onClose" :workspace-id="workspace?.id ?? ''" />
    </AlertDialogContent>
  </AlertDialog>
</template>
