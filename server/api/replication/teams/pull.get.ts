...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (checkpoint) {
      conditions.push(
        or(
          gt(tables.team.updated_at, new Date(checkpoint.updated_at)),
          and(
            sql`${tables.team.updated_at} = ${checkpoint.updated_at}::timestamp with time zone`,
            gt(tables.team.id, checkpoint.id),
          ),
        ),
      );
    }
    ...
    const rows = await db
      ...
      .orderBy(asc(tables.team.updated_at), asc(tables.team.id))
      .limit(batchSize);
