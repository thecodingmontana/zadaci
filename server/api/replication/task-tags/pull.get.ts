...
export default defineEventHandler(async (event) => {
  try {
    ...
    console.log(
      "[rxdb-debug] task-tags/pull | userId:",
      userId,
      "| workspaceId:",
      workspaceId,
      "| batchSize:",
    ...
    const conditions = [inArray(tables.task_tags.task_id, taskIds)];

    if (checkpoint) {
      conditions.push(
        or(
          gt(tables.task_tags.updated_at, new Date(checkpoint.updated_at)),
          and(
            sql`${tables.task_tags.updated_at} = ${checkpoint.updated_at}::timestamp with time zone`,
            gt(tables.task_tags.id, checkpoint.id),
          ),
        ),
      );
    }
    ...
    const rows = await db
      ...
      .orderBy(asc(tables.task_tags.updated_at), asc(tables.task_tags.id))
      .limit(batchSize);
