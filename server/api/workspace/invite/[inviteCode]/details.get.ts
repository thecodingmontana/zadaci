  ...
  try {
    ...
    if (!inviteCode || typeof inviteCode !== "string") {
      throw createError({
        statusCode: 400,
        statusMessage:
          "Missing workspace inviteCode! We're unable to load the workspace invite data",
      });
    }
    if (!query.email || query.email === "undefined") {
      throw createError({
        statusCode: 400,
        statusMessage:
          "Missing email query! Email is required for an automatic joining of the workspace!",
      });
    }
    ...
    if (!workspace) {
      throw createError({
        statusCode: 400,
        statusMessage:
          "We couldn't be able to find the workspace for the invite! Kindly request a new workspace invite mail.",
      });
    }
